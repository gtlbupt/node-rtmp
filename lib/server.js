...
Thread.prototype.pushTask = function (task){
	console.log("Function[Thread.pushTask]");
...
/**
 * @brief: start RTMPServer
 * @param: port {Number|Optional} 
 */
RTMPServer.prototype.start = function(port){
    ...
    return this.server.listen(this.port, '0.0.0.0', 511);
};
/**
 * @brief: alias of start function
 */
RTMPServer.prototype.listen = RTMPServer.prototype.start;

/**
 * @brief: stop the RTMPServer
 * @param: cb {Function}
 */
RTMPServer.prototype.stop = function(cb){
	if(this.activeTimer){
 ...
 */
RTMPServer.prototype.onNetConnectionConnect = function(netConn){
...
RTMPServer.prototype.createServer = function(){
    ...
    this.server =  net.createServer(function(socket){
            ...
            netConn.on('BasicHeader', function(args){});
            netConn.on('MessageHeader', function(args){});
            netConn.on('chunkData', function(args){});

            netConn.on('close', function(err){
                debug('NetConnection[%s] close, err',netConn.clientId, err);
            });
            netConn.on('error', function(err){
                debug('NetConnection[%s] error',netConn.clientId, err);
            }); 
            socket.on('close', function(err){
                netConn.emit('close', err);         
            });
            socket.on('error', function(err){
                netConn.emit('error', err);         
            });
    });
};
...
Pipe.prototype.onVideo = function (msg){
	//console.log("Function[Pipe.onVideo] enter, subSize: %d", this.subscribers.length);
    // this.timeJitter && this.timeJitter.jitter(msg); 
...
Pipe.prototype.onPlay = function(stream){
	console.log("Function[Pipe.onPlay]");
	if (this.state == Pipe.STATE.INIT){
...
MessageQueue.prototype.shrink = function(){
 ...
 const RTMP_STATE_HANDSHAKE = 1;
 const RTMP_STATE_CONNECTED = 2;
...
NetStream.prototype.onCycle = function (interval){
    var count = 0;
    while(this.msgQueue.length > 0 && count++ < 5){
...
NetStream.prototype.onPublish = function (){
...
NetStream.prototype.acceptPublish   = function(){
   debug("Function[NetStream.acceptPublish enter"); 
   this.netConn.acceptPublish(this.streamId); 
   debug("Function[NetStream.acceptPublish leave"); 
};
...
NetConnection.prototype.onConnect = function NetConnOnConnect(){
	console.log('Function[%s] called', arguments.callee.name);
	if (this.server){
		if (this.role === NetConnection.Role.IN){
			this.server.emit('connect', this);
		}
		this.server.onNetConnectionConnect(this);
	}
};
...
NetConnection.prototype.onClose= function(err){
    ...
    // err Boolean: has_error
    debug("Function[NetConnection.onClose] called, error", err);
	...
	if(this.server){
		this.server.onNetConnectionClose(this);
	}
...
NetConnection.prototype.AMF0CmdMessage = function (chunkStream) {
    ...
    try{
       ...
       //NetConn CMD
       case 'connect':
           debug('Function[AMF0CmdMessage] clientId[%s], command[%s]', this.clientId, cmdName);
           this.emit('connect', chunkStream.streamId, args);
           var cmdObj = args[2];
           var app = cmdObj.app.replace(/\/$/, '');
           this.setApp(app);
		   debug('Function[AMF0CmdMessage] clientId[%s], appName: %s', this.clientId, app);;
           break;
       case 'call':
       ...
       case 'close':
           this.emit(args[0], chunkStream.streamId, args);
            break;
       case 'createStream':
           debug('AMF0Cmd: createStream');
           var ns = this.createNetStream(); 
           this.emit(cmdName, ns);
      ...
      //NetStream CMD
       case 'play':
           ...
           var req = {
                vhost: ns.vhost,
                app:   ns.app,
                streamName: ns.streamName
           };
           debug('publishPool req, get', req);
		   var pipe = pool.get(req);
           ns.emit('play', pipe !== null, req, pool, args);
		   if (!pipe){
           ...
           (function(){
               var objs = [
                ...
                {type: amf0Types.kObjectType, value: 
                    [
                    {type:amf0Types.kStringType,  key:"code",   value: "NetStream.Publish.Start"},
                    {type:amf0Types.kStringType,  key:"description",  value: "Stream is Not publish"},
               ...
               var msg = Buffer.concat(chunks);
               var info = { fmt: 0, csid: 3, msg: msg, msgLength: msg.length, streamId: ns.streamId};
               buffer = this.encoder.AMF0CmdMessage(info);
               this.socket.write(buffer);
           }).call(this);
           break;
...
NetConnection.prototype.acceptConnect = function(){
       ...
       (function(){
        ...
        {type:amf0Types.kObjectType, value: 
            ...
            {type:amf0Types.kECMAArrayType, key: "data", value: 
                [
                {type:amf0Types.kStringType, key:"version", value:"3,5,3,888"}, 
                ]
            }
            ]
        }
       ...
       {type: amf0Types.kObjectType, value: 
           [
           {type: amf0Types.kStringType, key:"bmsVer", value:"BMS/0.14.306"},
           //{type: amf0Types.kNumberType, key:"capabilities", value: 31},
           {type: amf0Types.kNumberType, key:"mode", value: 1}
           ]
       },
       {type: amf0Types.kObjectType, value: 
           ...
           {type: amf0Types.kStringType, key:"code",  value:"NetConnection.Connect.InvalidApp"}
           ]
       }
       ]; 
       */
