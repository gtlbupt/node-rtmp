...
module.exports.PublishPool      = PublishPool;
...
function Pipe(opts){
    ...
    this.on('stopPublish',  Pipe.prototype.onStopPublish.bind(this));
...
Pipe.prototype.hasPublish = function(){
    return this.state === Pipe.STATE.PUBLISH; 
};
...
Pipe.prototype.onPublish = function (req, stream){
    console.log("Function[Pipe.onPublish] enter, state: %d", this.state);
    stream.pipe = this;
    this.publisher = stream;
    this.pubSize++;
    this.state = Pipe.STATE.PUBLISH;
    console.log("Function[Pipe.onPublish] state: %d, subscribers length: %d", this.state, this.subscribers.length);
    console.log("Function[Pipe.onPublish] leave, state: %d", this.state);
};

Pipe.prototype.onStopPublish = function(req, stream){
    console.log("Function[Pipe.onStopPublish] enter, state: %d, subSize: %d", this.state, this.subscribers.length);

    assert.equal(this.state, Pipe.STATE.PUBLISH);

    this.state = Pipe.STATE.STOP_PUBLISH;
    this.subscribers.forEach(function(stream){
        stream.stopPublish(function(){ stream.netConn.close(); });
    });
    this.publisher = null;
    this.pubSize--;
    this.subscribers = []; // clear subscribers array
    this.subSize     = 0;
    this.state = Pipe.STATE.INIT;

    console.log("Function[Pipe.onStopPublish] leave, state: %d, subSize: %d", this.state, this.subscribers.length);
};
...
Pipe.prototype.onStopPlay = function (req, stream){
    console.log("Function[Pipe.onStopPlay] enter.");
    this.removeSubscriber(req, stream);
    console.log("Function[Pipe.onStopPlay] leave.");
};

Pipe.prototype.onClose = function (req, stream){
    console.log("Function[Pipe.onClose] enter. pub size[%d] sub size[%d]", this.pubSize, this.subSize);
    ...
    console.log("Function[Pipe.onClose] leave. pub size[%d] sub size[%d]", this.pubSize,  this.subSize);
};

Pipe.prototype.removeSubscriber = function (req, stream){
    console.log("Function[removeSubscriber] enter, sub number: %d", this.subscribers.length);
    ...
    console.log("Function[removeSubscriber] leave, sub number: %d", this.subscribers.length);
    return false;
};
...
Pipe.prototype.appendSubscriber = function (stream){
    ...
    stream.waitForKeyFrame = (this.msgQueue.size() > 0) ? false : true;
    stream.startCycle();
...
function PublishPool(opts){
    ...
    this.on('stopPublish',  PublishPool.prototype.onStopPublish.bind(this));
    this.on('play',         PublishPool.prototype.onPlay.bind(this));
    this.on('stopPlay',     PublishPool.prototype.onStopPlay.bind(this));
}
inherits(PublishPool, require('events').EventEmitter);

PublishPool.prototype.onPublish = function (req, stream){
    console.log("Function[PublishPool.onPublish] enter, req %s, size: %d, isEdge: %d", JSON.stringify(req), this.size, this.server.isEdge);
    var pipe = this.get(req);
    if (!pipe) {
        pipe = new Pipe(req);
        this.set(req, pipe);
    }
    pipe.onPublish(null, stream);
    if (this.server.isEdge) {
        pipe.edgePublisher = new EdgePublish({server: this.server, pipe: pipe});
        var os = this.server.edge.origin_servers;
        console.log("Function[PublishPool.onPublish], origin_servers", os);
        pipe.edgePublisher.connect({
            host: os[0].host,
            port: os[0].port,
        });
    }
    console.log("Function[PublishPool.onPublish] leave, req %s, size: %d", JSON.stringify(req), this.size);
};
...
PublishPool.prototype.onStopPublish = function (req, stream){
    console.log("Function[PublishPool.onStopPublish] enter, req %s, size: %d", JSON.stringify(req), this.size);
    var pipe = this.get(req);
    if (pipe){ // forward to Pipe
        pipe.onStopPublish(req, stream);
        this.del(req);
    }
    console.log("Function[PublishPool.onStopPublish] leave, req %s, size: %d", JSON.stringify(req), this.size);
};

PublishPool.prototype.onPlay = function (req, stream) {
    console.log("Function[PublishPool.onPlay] enter, req %s, size: %d", JSON.stringify(req), this.size);
    var pipe = this.get(req);
    if (!pipe) {
        console.log("Function[PublishPool.onPlay] Not Found Pipe, req[%s], isEdge[%d]", JSON.stringify(req), this.server.isEdge);
        pipe = new Pipe(req);
        this.set(req, pipe);
        if (this.server.isEdge) {
            var os = this.server.edge.origin_servers;
            pipe.edgePlay = new EdgePlay({server: this.server, pipe: pipe, vhost: stream.vhost, app: stream.app, streamName: stream.streamName});
            pipe.edgePlay.connect({ host: os[0].host, port: os[0].port}, function(err){
                console.log("Function[PublishPool.onPlay] edgePlay connect origin-server failed. error: ", err);
    ...
    // notify Pipe
    pipe.onPlay(stream);
    console.log("Function[PublishPool.onPlay] leave, req %s, size: %d", JSON.stringify(req), this.size);
};

PublishPool.prototype.onStopPlay = function(req, stream) {
    console.log("Function[PublishPool.onStopPlay] enter, req %s, size: %d", JSON.stringify(req), this.size);
    var pipe = this.get(req);
    if (pipe) {
        pipe.onStopPlay(req, stream);
    }
    console.log("Function[PublishPool.onStopPlay] enter, req %s, size: %d", JSON.stringify(req), this.size);
};
...
PublishPool.prototype.set = function(req, pipe){
    ...
    console.log("Function[PublishPool.set] enter. key=[%s]", key);
...
function NetConnection(opts) {
    ...
    this.once('error',  NetConnection.prototype.onError.bind(this));
 ...
 */
NetConnection.prototype.connect = function connect(opts, callback){
...
function NetStream(opts){
    ...
    this.on('stopPublish',  NetStream.prototype.onStopPublish.bind(this));
    this.on('play',         NetStream.prototype.onPlay.bind(this));
    this.on('stopPlay',     NetStream.prototype.onStopPlay.bind(this));
...
NetStream.prototype.handlePublish = function(args){
    console.log("Function[NetStream.handlePublish] enter.");
    var streamName = args[3].split('?')[0];
    ...
    if (pipe) {
        console.log("Function[NetStream.handlePublish] enter. found pipe");
    } else {
        console.log("Function[NetStream.handlePublish] enter. not found pipe");
    }
    if (pipe && pipe.hasPublish()) {
        hasPublish = true;
    }
    // Notify User 
    this.emit('publish', args, hasPublish);
    console.log("Function[NetStream.handlePublish] leave.");
};
...
NetStream.prototype.onPublish = function (args){
    ...
    console.log("Function[NetStream.onPublish] leave state: %d.", this.state);
};

NetStream.prototype.onStopPublish = function (){
    console.log("Function[NetStream.onStopPublish] state: %d", this.state);
    assert(this.state === NetStream.STATE.PUBLISH);
    this.state = NetStream.STATE.STOP_PUBLISH;
    var pool = this.netConn.server.publishPool;
    if (pool) {
        var req = { vhost: this.vhost, app: this.app, streamName: this.streamName};
        pool.onStopPublish(req, this);
        console.log("Function[NetStream.onStopPublish] leave state: %d, req: %s", this.state, JSON.stringify(req));
    } else {
        console.log("Function[NetStream.onStopPublish] leave state: %d, not found publishPool", this.state);
    }
};


NetStream.prototype.handlePlay = function(args){
    console.log("Function[NetStream.handlePlay] enter.");
    var streamName = args[3];
    this.pause(false, 0);
    this.setName(streamName);
    this.setRolePlay();
    this.emit('play', args);
    console.log("Function[NetStream.handlePlay] leave.");
};
...
NetStream.prototype.onPlay = function(args){
    ...
    this.state = NetStream.STATE.PLAY;
    console.log("Function[NetStream.onPlay] leave");
};

NetStream.prototype.onPlay2 = function (args){
    this.setRolePlay();
    this.emit('play2', args);
};

NetStream.prototype.onStopPlay = function(args){
    this.state = NetStream.STATE.STOP_PLAY;
};

NetStream.prototype.handleOnStatus = function(args){
    this.emit('onStatus', args);
};
...
NetStream.prototype.onClose = function onClose(err){
    ...
    switch(this.state){
        ...
        case NetStream.STATE.STOP_PUBLISH:
            var pool = this.netConn.server.publishPool;
            var req  = {vhost: this.vhost, app: this.app, streamName: this.streamName};
            pool.onStopPublish(req, this);
            break;
        case NetStream.STATE.PLAY:
        case NetStream.STATE.STOP_PLAY:
            var pool = this.netConn.server.publishPool;
            var req = {vhost: this.vhost, app: this.app, streamName: this.streamName};
            pool.onStopPlay(req, this);
            break;
        default:
            break;
    }
...
NetStream.prototype.acceptPublish = function(args){
    console.log("Function[NetStream.acceptPublish] enter.");
    // (1) send NetConnection.Status.Success
    this.netConn.acceptPublish(this.streamId);
    // (2) notify PublishPool
    var pool = this.netConn.server.publishPool;
    var req  = {vhost: this.vhost, app: this.app, streamName: this.streamName};
    pool.onPublish(req, this);
    console.log("Function[NetStream.acceptPublish] leave.");
};

/**
 * @brief: first acceptPlay then pool.onPlay
 */
NetStream.prototype.acceptPlay = function(){
    console.log("Function[NetStream.acceptPlay] enter.");
    this.netConn.acceptPlay(this.streamId);

    var pool = this.netConn.server.publishPool;
    var req  = {vhost: this.vhost, app: this.app, streamName: this.streamName};
    console.log("Function[NetStream.acceptPlay] req[%s].", JSON.stringify(req));
    pool.onPlay(req, this);
    console.log("Function[NetStream.acceptPlay] leave.");
};
...
NetConnection.prototype.send = function send(c){
    ...
    var self = this;
    if(this.canSend && !(this.state & RTMP_STATE_CLOSED)){
...
NetConnection.prototype.handleConnect = function handleConnect(chunkStream, args){
    console.log("Function[NetConnection.handleConnect] enter. clientId[%s]", this.clientid);
    // notify
    this.emit('connect', args);
    console.log("Function[NetConnection.handleConnect] leave. clientId[%s]", this.clientid);
};
 ...
 * @brief: set appName 
 */
NetConnection.prototype.onConnect = function onConnect(args){
...
NetConnection.prototype.handleClose = function handleClose(chunkStream, args){
    console.log("Function[NetConnection.handleClose] enter. clientId[%s]", this.clientid);
    this.emit('close', args);
    console.log("Function[NetConnection.handleClose] leave. clientId[%s]", this.clientid);
};
...
NetConnection.prototype.handleCreateStream = function handleCreateStream(chunkStream, args){
    console.log("Function[NetConnection.handleCreateStream] enter. clientId[%s]", this.clientid);
    this.onCreateStream(args);
    console.log("Function[NetConnection.handleCreateStream] leave. clientId[%s]", this.clientid);
};
...
NetConnection.prototype.handleCloseStream = function handleCloseStream(chunkStream, args){
    console.log("Function[NetConnection.handleCloseStream] enter. clientId[%s]", this.clientid);
    var streamId = chunkStream.streamId;
    this.onCloseStream(streamId);
    console.log("Function[NetConnection.handleCloseStream] leave. clientId[%s]", this.clientid);
};

NetConnection.prototype.handleDeleteStream = function handleDeleteStream(chunkStream, args){
    console.log("Function[NetConnection.handleDeleteStream] enter. clientId[%s]", this.clientid);
    var streamId = args[3];
    this.onCloseStream(streamId);
    console.log("Function[NetConnection.handleDeleteStream] leave. clientId[%s]", this.clientid);
};

NetConnection.prototype.onCloseStream = function(streamId){
    console.log('Function[NetConnection.onCloseStream] enter, clientId[%s]', this.clientId);
    var ns = this.findNetStreamById(streamId);
    if (ns) {
        ns.onClose();
        this.deleteNetStreamById(streamId);
    }
    console.log('Function[NetConnection.onCloseStream] leave, clientId[%s]', this.clientId);
};

/********************************************************/

NetConnection.prototype.handleReleaseStream = function (chunkStream, args){
    console.log("Function[NetConnection.handleReleaseStream] enter.");
    this.onReleaseStream(args);
    console.log("Function[NetConnection.handleReleaseStream] leave.");
};

NetConnection.prototype.onReleaseStream = function (args){
    console.log('Function[NetConnection.onReleaseStream] enter, clientId[%s]', this.clientId);
    ...
    console.log('Function[NetConnection.onReleaseStream] leave, clientId[%s]', this.clientId);
};

NetConnection.prototype.handleFCPublish = function handleFCPublish(chunkStream, args){
    console.log("Function[NetConnection.handleFCPublish] enter.");
    this.onFCPublish(args);
    console.log("Function[NetConnection.handleFCPublish] leave.");
};

NetConnection.prototype.onFCPublish = function (args) {
    console.log('Function[NetConnection.onFCPublish] enter, clientId[%s]', this.clientId);
    ...
    console.log('Function[NetConnection.onFCPublish] leave, clientId[%s]', this.clientId);
};

NetConnection.prototype.onFCUnpublish = function(args, streamId){
    console.log('Function[NetConnection.onFCUnpublish] enter, clientId[%s]', this.clientId);
    ...
    (function(){
        ...
        {type: amf0Types.kObjectType, value: [
            ...
            {type:amf0Types.kStringType,  key:"description",  value: "stop publish stream"},
        ...
        var msg = Buffer.concat(chunks);
        var info = { fmt: 0, csid: 3, msg: msg, msgLength: msg.length, streamId: streamId};
        var buffer = this.encoder.AMF0CmdMessage(info);
        this.socket.write(buffer);
    }).call(this);

    // response _result(FCUnpublish) 
    (function(){
        var transId = args[1];
        var objs = [
        {type: amf0Types.kStringType, value: "_result"},
    ...
    //this.emit('FCUnpublish'); 
    console.log('Function[NetConnection.onFCUnpublish] leave, clientId[%s]', this.clientId);
};

NetConnection.prototype.onError = function(err){
    console.log('Function[NetConnection.onError] enter, clientId[%s]', this.clientId, err);
    this.onClose();
    console.log('Function[NetConnection.onError] leave, clientId[%s]', this.clientId);
};
...
NetConnection.prototype.acceptPublish = function(streamId){
    console.log('Function[NetConnection.acceptPublish] enter, clientId[%s]', this.clientId);
    (function(){
        var objs = [
    ...
    console.log('Function[NetConnection.acceptPublish] leave, clientId[%s]', this.clientId);
};

NetConnection.prototype.acceptPlay = function(streamId){
    console.log('Function[NetConnection.acceptPlay] enter, clientId[%s]', this.clientId);
    //(0) SetChunkSize
    ...
    (function(){
        ...
        var msg = Buffer.concat(chunks);
        var info = {fmt: 0, csid: 0x03, msg: msg, msgLength: msg.length, streamId: streamId};
        var buffer = this.encoder.AMF0CmdMessage(info);
        this.socket.write(buffer);
    }).call(this);
    ...
    console.log('Function[NetConnection.acceptPlay] leave, clientId[%s]', this.clientId);
}
...
NetConnection.prototype._onMessageHdr = function(buf){
    ...
    var chunkStream = this.chunkStream[this.csid] ||
        (this.chunkStream[this.csid] = new ChunkStream({fmt: this.fmt, csid: this.csid}) );
        /**
...
NetConnection.prototype._onExtendTimestamp = function(buf){
    ...
    if(chunkStream.fmt == 0){
        ...
        chunkStream.absTimestamp += time;
    } else { //chunkStream.fmt == 3
         if (time !== chunkStream.absTimestamp){ // FMLE extended-timestamp >= 0xffffff,  4h40min
            chunkStream.buf = buf; 
         } // no-extended-timestamp in chunkStream.fmt = 3;
    }

    debug("Function[_onChunkData] clientId[%s] enter, fmt: %d, time:%d", this.clientId, chunkStream.fmt, time);

    var toReadSize = Math.min(chunkStream.msgLength - chunkStream.chunkReadSize - chunkStream.buf.length, this.inChunkSize);
    if ( toReadSize < this.inChunkSize) {
        this._bytes(toReadSize, this._onChunkData);       
    } else {
        this._bytes(this.inChunkSize, this._onChunkData);       
...
NetConnection.prototype._onChunkData = function(buf){
    ...
    if (chunkStream.buf.length > 0) {
        chunkStream.buf.copy(chunkStream.msg, chunkStream.chunkReadSize);
        chunkStream.chunkReadSize += chunkStream.buf.length;
        chunkStream.buf = new Buffer(0);
    }
...
NetConnection.prototype.AMF0CmdMessage = function (chunkStream) {
    ...
    try{
        ...
        switch(cmdName){
            ...
            case 'connect':
                //this.emit('connect', args);
                this.handleConnect(chunkStream, args);
                break;
            case 'close':
                //this.emit('close', args);
                this.handleClose(chunkStream, args);
                break;
            case 'createStream': // createStream
                //this.onCreateStream(args);
                this.handleCreateStream(chunkStream, args);
                break;
            case 'closeStream': // closeStream
                 ...
                 * this.onCloseStream(streamId);
                 **/
                this.handleCloseStream(chunkStream, args);
                break;
            case 'deleteStream': /** {"deleteStream", 5, NUll, 1} */
                /**
                 * var streamId = args[3];
                 * this.onCloseStream(streamId);
                 **/
                this.handleDeleteStream(chunkStream, args);
                break;
            case '_checkbw':
                // ignore
                break;
            ...
            case 'pause':
                ...
                break;
            case 'releaseStream': // {'releaseStream', 2, null, 'livestream'}
                //this.onReleaseStream(args);
                this.handleReleaseStream(chunkStream, args);
                break;
            case 'FCPublish': // {'FCPublish', 3, null, 'livestream'}
                //this.onFCPublish(args);
                this.handleFCPublish(chunkStream, args);
                break;
            case 'FCUnpublish':
                this.onFCUnpublish(args, chunkStream.streamId);
                var ns = this.findNetStreamByName(args[3]);
                if (ns) {
                    ns.onStopPublish(args);
                } else {
                    console.error("Function[NetConnection.FCUnpublish] not found NetStream. streamId[%s] args=[%s]", chunkStream.streamId, JSON.stringify(args));
                }
                break;
            case '_result':
                var callback = this.stub[transId];
                if(callback){
                    debug("_result argument", args);
                    console.log("_result argument", JSON.stringify(args));
                    callback(null, args[3]);
                    this.stub[transId] = null;
                }
                break;
            case 'onStatus':
...
NetConnection.prototype.acceptConnect = function(){
    console.log('Function[NetConnection.acceptConnect] enter.');
    ...
    (function(){
        var buffer = this.encoder.UserControlMessageEvents(0x00, 0);
        this.socket.write(buffer);
    }).call(this);
    ...
    console.log('Function[NetConnection.acceptConnect] leave.');
};
 ...
 * @param: opts.port,
 * @param: opts.app,
 * @param: opts.streamName,
 * @param: opts.source {NetStream},
 * @param: callback {Function}
 */
EdgePublish.prototype.connect = function (opts, callback){
    console.log("Function[EdgePublish.connect] enter.");
    ...
    this.on('stopPublish',  EdgePublish.prototype.onStopPublish.bind(this));
    ...
    this.forwarder.on('__createStream', function(stream){
        ...
        stream.publish(self.pipe.streamName, function (err){
            if (err) {
                console.log("Function[__createStream] publish failed, streamName", self.pipe.streamName);
                callback(err);
                return; 
            }
        });
...
EdgePublish.prototype.onPublish     = function(stream){
    console.log("Function[EdgePublish] enter.");
    this.stream = stream;
    this.state = EdgePublish.STATE.PUBLISHED;
    this.pipe.onPlay(stream);
    console.log("Function[EdgePublish] leave.");
};

EdgePublish.prototype.onStopPublish = function(stream){
    console.log("Function[EdgePublish.onStopPublish] enter.");
    this.state = EdgePublish.STATE.INIT;
    this.pipe.onStopPlay(null, stream);
    // close Ourself.
    this.netConn.emit('close');
    console.log("Function[EdgePublish.onStopPublish] leave.");
};
...
EdgePlay.prototype.connect = function(opts, callback){ console.log("Function[EdgePlay.connect] enter, opts: %s", JSON.stringify(opts));
    ...
    this.on('play', EdgePlay.prototype.onPlay.bind(this));
    this.on('stopPlay', EdgePlay.prototype.onStopPlay.bind(this));
...
EdgePlay.prototype.onPlay = function(stream){
    console.log("Function[EdgePlay.onPlay] enter.");
    this.stream = stream;
    this.state  = EdgePlay.STATE.PLAY;
    var req     = {vhost: this.vhost, app: this.app, streamName: this.streamName};
    this.pipe.onPublish(req, stream);
    console.log("Function[EdgePlay.onPlay] leave.");
};

EdgePlay.prototype.onStopPlay = function(stream) {
    console.log("Function[EdgePlay.onStopPlay] enter.");
    if (this.state != EdgePlay.STATE.PLAY) {
        console.log("Function[EdgePlay.onStopPlay] repeate stopPlay.");
        return;
    }
    this.state = EdgePlay.STATE.INIT;
    this.pipe.onStopPublish(null, stream);
    this.netConn.emit('close');
    console.log("Function[EdgePlay.onStopPlay] leave.");
};
...
/*  vim: set expandtab tw=79 ts=4 sts=4 sw=4: */
