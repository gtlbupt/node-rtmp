...
PublishPool.prototype.onPlay = function (req, stream) {
    ...
    var pipe = this.get(req);
    if (!pipe) {
        console.log("Function[PublishPool.onPlay] Not Found Pipe", JSON.stringify(req));
        pipe = new Pipe(req);
        this.set(req, pipe);
        if (this.server.isEdge) {
            pipe.edgePlay = new EdgePlay({server: this.server, pipe: pipe, vhost: ns.vhost, app: ns.app, streamName: ns.streamName});
            pipe.edgePlay.connect({ host: 'cp01-wise-2011q4ecom05.cp01.baidu.com', port: 8935});
        }
    }
    pipe.onPlay(stream);
...
NetStream.prototype.stopPublish = function (cb){
    ...
    // (1) send FCUnpublish
    (function(){
        console.log("Function[NetStream.stopPublish] m1, state: %d", this.state);
        var objs = [
        ...
        console.log("Function[NetStream.stopPublish] m2, state: %d", this.state);
        var chunks = [];
        objs.forEach(function(val, idx, arr){
            chunks.push(amf.write(val));
        });
        console.log("Function[NetStream.stopPublish] m3, state: %d", this.state);
        var msg = Buffer.concat(chunks);
        console.log("Function[NetStream.stopPublish] m3, state: %d", this.state);
        ...
        console.log("Function[NetStream.stopPublish] FCUnpublish, state: %d", this.state);
    }.call(this));
    ...
    (function(){
        ...
        var buffer = this.netConn.encoder.AMF0CmdMessage(info);
        this.netConn.socket.write(buffer);
        console.log("Function[NetStream.stopPublish] deleteStream, state: %d", this.state);
    }.call(this));
...
EdgePublish.prototype.onStopPublish = function(stream){
    console.log("EdgePublish onStopPublish");
    this.state = EdgePublish.STATE.INIT;
};
